...
# # make networks undirectional 
# el_copy = copy(el)
# setnames(el_copy, old=c('node_i', 'node_j'), new=c('node_j', 'node_i'))
# el = rbindlist(list(el, el_copy), use.names = T)
# nrow(el)
# 
# # find dups
...
foreach(s = 1:max_step, .packages = c('data.table', 'igraph'), .combine = 'c') %dopar%  {
  ...
  if(s<10){
    save(output,file= paste0("output/20230803/", el_name, '_step_000', s, ".rdata"))
  } else if(s<100){
    save(output,file= paste0("output/20230803/", el_name, '_step_00', s, ".rdata"))
  } else if(s<1000){
    save(output,file= paste0("output/20230803/", el_name, '_step_0', s, ".rdata"))
  } else{
    save(output,file= paste0("output/20230803/", el_name, '_step_', s, ".rdata"))
  }
